    ...
    <div className="flex gap-0.5">
      {[1,2,3,4,5].map(s => (
        <button key={s} type="button" onClick={() => onChange?.(s)}
          ...
          <Star size={size}
            className={s <= (hovered || value) ? 'text-amber-400' : 'text-white/60'}
...
export default function ProfileDetailPage() {
  ...
  if (!profile) return (
    <div className="min-h-screen bg-[#111] flex items-center justify-center text-center px-4">
      <div>
        <p className="text-white/40 mb-4">Profil introuvable.</p>
  ...
  return (
    <div className="min-h-screen bg-[#111] text-white">
      ...
      <div className="bg-[#0d0d0d] border-b border-gray-800">
        <div className="max-w-7xl mx-auto px-4 py-2 flex items-center gap-1.5 text-xs text-white/60 overflow-x-auto whitespace-nowrap">
          ...
          <span>/</span>
          <span className="text-white/50 truncate max-w-[120px]">{profile.displayName}</span>
        </div>
      </div>
        ...
        <div className="relative overflow-hidden cursor-zoom-in group"
          ...
          {photos[selectedPhoto] ? (
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-gray-900 to-[#1a1a1a]">
              <span className="text-9xl font-bold text-white/70">{profile.displayName[0]}</span>
            </div>
          )}
          ...
          <div className="absolute top-3 left-3 flex gap-2">
            ...
            {profile.isVerified && (
              <span className="flex items-center gap-1 bg-emerald-900/200 text-white text-[10px] font-black px-2.5 py-1 rounded-md shadow-lg shadow-emerald-500/40 uppercase tracking-wide">
          ...
          <div className="absolute top-3 right-3 flex flex-col items-end gap-1.5">
            {profile.isOnline ? (
              <span className="flex items-center gap-1.5 bg-emerald-900/200 text-white text-[10px] font-bold px-2.5 py-1 rounded-md shadow-lg uppercase">
                <span className="w-1.5 h-1.5 bg-white rounded-full animate-pulse" /> En ligne
              </span>
            ) : (
              <span className="flex items-center gap-1.5 bg-black/60 backdrop-blur text-white/40 text-[10px] px-2.5 py-1 rounded-md uppercase">
                <span className="w-1.5 h-1.5 bg-white/50 rounded-full" /> Hors ligne
              </span>
            )}
            <span className="flex items-center gap-1 bg-black/50 backdrop-blur text-white/40 text-[10px] px-2.5 py-1 rounded-md">
          ...
          <div className="lg:col-span-2 space-y-4">
              ...
              {isAuthenticated && (
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={toggleFavorite}
                    className={`flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-medium border transition-all ${favorited ? 'bg-brand-500/15 border-brand-500/40 text-brand-300' : 'border-gray-700 text-white/50 hover:border-gray-600'}`}>
                  ...
                  <button onClick={() => { navigator.clipboard.writeText(window.location.href); toast.success('Lien copié !'); }}
                    className="flex items-center justify-center gap-2 py-3 rounded-xl text-sm text-white/50 border border-gray-700 hover:border-gray-600">
                    <Copy size={14} /> Copier
                  </button>
                </div>
              )}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {[
                ...
                <div key={label} className={`rounded-xl p-3 text-center border ${highlight ? 'bg-emerald-900/200/10 border-emerald-500/20' : 'bg-[#1a1a1a] border-gray-800'}`}>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {profile.tags.map((t: string) => (
                      <Link key={t} href={`/profiles?tags=${t}`}
                        className="text-[11px] bg-gray-800 text-white/40 hover:text-white border border-gray-700/50 hover:border-gray-600 px-2.5 py-1 rounded-full transition-colors">
                        #{t}
                      </Link>
                    ))}
                  </div>
            ...
            {profile.bio && (
              <div className="bg-[#1a1a1a] border border-gray-800 rounded-xl p-5">
                ...
                <p className={`text-white/40 text-sm leading-relaxed ${!bioExpanded && profile.bio.length > 280 ? 'line-clamp-4' : ''}`}>
                  {profile.bio}
                </p>
                  ...
                  {photos.length > 0 && (
                    <button onClick={() => setActiveMediaTab('photos')}
                      className={`flex items-center gap-2 px-5 py-3 text-sm font-medium border-b-2 transition-colors ${
                        activeMediaTab === 'photos' ? 'text-white border-brand-500 bg-brand-500/5' : 'text-white/50 border-transparent hover:text-white/30'
                      }`}>
                      <Camera size={13} /> Photos <span className="text-xs text-white/60">({photos.length})</span>
                    </button>
                  )}
                  {videos.length > 0 && (
                    <button onClick={() => setActiveMediaTab('videos')}
                      className={`flex items-center gap-2 px-5 py-3 text-sm font-medium border-b-2 transition-colors ${
                        activeMediaTab === 'videos' ? 'text-white border-brand-500 bg-brand-500/5' : 'text-white/50 border-transparent hover:text-white/30'
                      }`}>
                      <Video size={13} /> Vidéos <span className="text-xs text-white/60">({videos.length})</span>
                    </button>
                  )}
                </div>
                        ...
                        {videos.map((v: any) => (
                          <div key={v.id} className="relative aspect-video rounded-lg overflow-hidden bg-gray-900 group cursor-pointer">
                            {v.thumbnailUrl
                              ...
                              : <div className="w-full h-full flex items-center justify-center"><Video size={24} className="text-white/60" /></div>
                            }
            ...
            <div className="bg-[#1a1a1a] border border-gray-800 rounded-xl p-5">
              <div className="flex items-center justify-between mb-4">
                  ...
                  {avgRating > 0 && (
                    <div className="flex items-center gap-1.5">
                      ...
                      <div>
                        ...
                        <p className="text-white/60 text-[10px]">{reviews.length} avis</p>
                      </div>
                    </div>
                  )}
                </div>
                {isAuthenticated && !alreadyReviewed && !showReviewForm && (
                  <div className="flex flex-col items-end gap-1">
                    ...
                    {!hasMessaged && (
                      <p className="text-[10px] text-white/60">💬 Contactez pour un avis vérifié</p>
                    )}
                  </div>
                )}
              </div>
              ...
              {showReviewForm && (
                <div className="bg-[#111] border border-gray-700/50 rounded-xl p-4 mb-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {hasMessaged ? (
                      <span className="text-[10px] bg-emerald-900/200/10 text-emerald-400 border border-emerald-500/20 px-2 py-0.5 rounded-full">✓ Avis vérifié</span>
                    ) : (
                      <span className="text-[10px] bg-amber-900/200/10 text-amber-400 border border-amber-500/20 px-2 py-0.5 rounded-full">💬 Non vérifié</span>
                    )}
                  </div>
                  <StarRating value={review.rating} onChange={v => setReview(r => ({ ...r, rating: v }))} size={22} />
                  <textarea
                    className="w-full mt-3 bg-[#1a1a1a] border border-gray-700 rounded-xl px-3 py-2.5 text-white/30 text-sm placeholder-gray-600 focus:outline-none focus:border-brand-500 resize-none"
                    rows={3} placeholder="Partagez votre expérience..." value={review.comment}
                    onChange={e => setReview(r => ({ ...r, comment: e.target.value }))} />
                  <div className="flex gap-2 mt-3">
                    <button onClick={() => setShowReviewForm(false)} className="flex-1 border border-gray-700 text-white/40 py-2.5 rounded-xl text-sm hover:text-white transition-colors">Annuler</button>
                    ...
                    <div key={i} className="border-t border-gray-800 pt-3 first:border-0 first:pt-0">
                      <div className="flex items-start justify-between gap-3 mb-1.5">
                        <div className="flex items-center gap-2">
                          <div className="w-7 h-7 bg-gray-800 rounded-full flex items-center justify-center text-xs font-bold text-white/40">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              {r.isVerified && (
                                <span className="text-[9px] bg-emerald-900/200/10 text-emerald-400 px-1.5 py-0.5 rounded-full border border-emerald-500/20 font-bold">✓ Vérifié</span>
                              )}
                            </div>
                          </div>
                        </div>
                      ...
                      {r.comment && <p className="text-white/40 text-xs leading-relaxed pl-9">{r.comment}</p>}
                    </div>
                  ))}
                </div>
              )}
              ...
              {!isAuthenticated && (
                <div className="mt-4 pt-4 border-t border-gray-800 text-center">
                  <p className="text-white/50 text-xs mb-2">Connectez-vous pour laisser un avis</p>
                  <div className="flex gap-2 justify-center">
                    <Link href={`/auth/login?redirect=/profiles/${id}`} className="text-xs border border-gray-700 text-white/40 hover:text-white px-4 py-1.5 rounded-lg transition-colors">Se connecter</Link>
          ...
          <div className="space-y-4 lg:sticky lg:top-20 self-start">
              ...
              <div className="p-4 space-y-2.5">
                ...
                {isAuthenticated ? (
                  ...
                  <div className="space-y-2">
                    <div className="bg-[#111] border border-gray-700/50 rounded-xl p-4 text-center">
                      ...
                      <p className="text-[11px] text-white/50 mb-3">Créez un compte gratuit pour envoyer des messages</p>
                      <div className="flex gap-2">
                        <Link href={`/auth/login?redirect=/profiles/${id}`}
                          className="flex-1 text-center text-xs font-semibold text-white/30 border border-gray-700 py-2 rounded-lg hover:border-gray-500 transition-colors">
                          Connexion
                        </Link>
                        <Link href="/auth/register"
                ...
                {isAuthenticated && (
                  <button onClick={toggleFavorite}
                    className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium border transition-all ${
                      favorited ? 'bg-brand-500/10 border-brand-500/30 text-brand-300' : 'border-gray-700 text-white/50 hover:border-gray-600 hover:text-white/30'
                    }`}>
                    <Heart size={14} fill={favorited ? 'currentColor' : 'none'} />
                ...
                <div className="grid grid-cols-3 gap-1.5 pt-1">
                  ...
                  <button onClick={() => window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(window.location.href)}`, '_blank', 'width=600,height=400')}
                    className="flex flex-col items-center gap-1 py-2.5 rounded-lg border border-gray-800 hover:bg-blue-900/200/10 hover:border-blue-500/30 transition-all">
                    ...
                    <span className="text-[9px] text-white/60">Facebook</span>
                  </button>
                  <button onClick={() => { navigator.clipboard.writeText(window.location.href); toast.success('Lien copié !'); }}
                    ...
                    <span className="text-[9px] text-white/60">Copier</span>
                  </button>
                </div>
              </div>
            </div>
              ...
              <div className="bg-[#1a1a1a] border border-gray-800 rounded-xl p-4 space-y-2">
                {profile.isVerified && (
                  <div className="flex items-center gap-2.5">
                    <div className="w-8 h-8 bg-emerald-900/200/10 rounded-lg flex items-center justify-center shrink-0">
                      <Shield size={14} className="text-emerald-400" />
                    </div>
                    <div>
                      <p className="text-white text-xs font-bold">Profil vérifié</p>
                      <p className="text-white/60 text-[10px]">Identité contrôlée par Afrodite</p>
                    </div>
                  </div>
                )}
                {isBoosted && (
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-white/60 text-[10px]">Mise en avant prioritaire</p>
                    </div>
                  </div>
                )}
              </div>
                      ...
                      <Link key={s.id} href={`/profiles/${s.id}`}
                        ...
                        <div className="relative w-10 h-12 rounded-md overflow-hidden bg-gray-800 shrink-0">
                          {sPhoto
                            ...
                            : <div className="w-full h-full flex items-center justify-center text-white/60 font-bold text-sm">{s.displayName?.[0]}</div>
                          }
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {s.age && <p className="text-white/60 text-[10px]">{s.age} ans · {s.city}</p>}
                          {s.pricePerHour && <p className="text-brand-400 text-[10px] font-bold">{Number(s.pricePerHour).toLocaleString()} FCFA/h</p>}
                        </div>
                        <ChevronRight size={12} className="text-white/70 group-hover:text-brand-400 shrink-0" />
                      </Link>
                    );
                  })}
                </div>
            ...
            {isAuthenticated && (
              <button onClick={() => setShowReportModal(true)}
                className="w-full flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs text-white/70 hover:text-red-400 hover:bg-red-900/200/5 border border-transparent hover:border-red-500/20 transition-all">
        ...
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4" onClick={() => setShowReportModal(false)}>
          <div className="bg-[#1a1a1a] border border-gray-700 rounded-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setShowReportModal(false)} className="text-white/60 hover:text-white"><X size={18} /></button>
            </div>
            {reportDone ? (
              <div className="text-center py-6">
                <div className="w-14 h-14 bg-emerald-900/200/10 rounded-full flex items-center justify-center mx-auto mb-4">
                  <Check size={24} className="text-emerald-400" />
                </div>
                <h3 className="font-semibold text-white mb-2">Signalement envoyé</h3>
                <p className="text-sm text-white/40 mb-4">Notre équipe va examiner ce profil. Merci.</p>
              ...
              <div className="space-y-2">
                {[
                  ...
                  <label key={r.value} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${reportReason === r.value ? 'border-red-500/40 bg-red-900/200/5 text-white' : 'border-gray-800 text-white/40 hover:border-gray-700'}`}>
                ...
                <textarea className="w-full bg-[#0d0d0d] border border-gray-800 rounded-xl px-3 py-2.5 text-white/30 text-sm placeholder-gray-600 focus:outline-none focus:border-gray-600 resize-none mt-1"
                  rows={2} placeholder="Détails (optionnel)" value={reportDetails} onChange={e => setReportDetails(e.target.value)} />
                <div className="flex gap-2 pt-1">
                  <button onClick={() => setShowReportModal(false)} className="flex-1 border border-gray-700 text-white/40 py-2.5 rounded-xl text-sm hover:text-white">Annuler</button>
                  <button onClick={submitReport} disabled={reportSubmitting || !reportReason}
                    className="flex-1 bg-red-900/200 hover:bg-red-600 text-white font-bold py-2.5 rounded-xl text-sm disabled:opacity-50 flex items-center justify-center gap-1.5">
