...
const PLANS = [
  {
    ...
    bg: 'bg-amber-900/20 border-amber-200',
  ...
  },
  {
    ...
    bg: 'bg-purple-900/20 border-purple-200',
...
export default function BoostPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      {/* Boost actif */}
      {!loading && activeBoost && (
        <div className="card p-5 mb-8 bg-emerald-900/20 border-emerald-200 flex items-center gap-4">
          <div className="w-12 h-12 bg-emerald-900/200 rounded-xl flex items-center justify-center shrink-0">
      ...
      <div className="grid grid-cols-3 gap-4 mb-10">
        {[
          { icon: Eye, label: 'Plus de vues', value: '×10', color: 'text-blue-500 bg-blue-900/20' },
          { icon: TrendingUp, label: 'Position', value: 'Top 1', color: 'text-emerald-500 bg-emerald-900/20' },
          { icon: Star, label: 'Page accueil', value: 'Vedette', color: 'text-amber-500 bg-amber-900/20' },
        ...
        {PLANS.map(({ plan, label, price, icon: Icon, color, bg, badge, features }) => {
          ...
          return (
            <div key={plan} className={`relative rounded-2xl p-6 border-2 bg-[#1a1a1a] ${isActive ? 'border-emerald-400 ring-2 ring-emerald-400/20' : 'border-white/10 hover:border-brand-400/40'} transition-all hover:-translate-y-1 hover:shadow-lg`}>
              ...
              {isActive && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-emerald-900/200 text-white text-xs font-bold px-3 py-1 rounded-full">✅ Actif</span>
              )}
              ...
              <button
                ...
                className={`w-full py-2.5 rounded-xl font-medium text-sm transition-all disabled:opacity-60 flex items-center justify-center gap-2 ${isActive ? 'bg-emerald-100 text-emerald-400 cursor-default' : 'bg-gradient-to-r ' + color + ' text-white hover:opacity-90 hover:shadow-md'}`}
              >
                {activating === plan ? (
