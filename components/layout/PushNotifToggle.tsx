...
export default function PushNotifToggle() {
  ...
  const configs = {
    loading:     { icon: Loader,    label: 'Chargement…',      cls: 'text-white/40',  spin: true  },
    granted:     { icon: BellRing,  label: 'Push activé',      cls: 'text-brand-400', spin: false },
    default:     { icon: Bell,      label: 'Activer les push',  cls: 'text-white/50',  spin: false },
    denied:      { icon: BellOff,   label: 'Push bloqué',       cls: 'text-red-400',   spin: false },
    unsupported: { icon: BellOff,   label: 'Non supporté',      cls: 'text-white/30',  spin: false },
  };
    ...
    <button
      ...
      className={`flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium border transition-all
        ${state === 'granted' ? 'bg-brand-50 border-brand-200 text-brand-600 hover:bg-brand-100' :
          state === 'denied' ? 'bg-red-900/20 border-red-500/20 text-red-400 cursor-not-allowed' :
          'bg-white/5 border-white/10 text-white/60 hover:border-brand-300 hover:bg-brand-50 hover:text-brand-600'
        }`}
    >
