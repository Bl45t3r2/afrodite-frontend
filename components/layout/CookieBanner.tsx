...
export default function CookieBanner() {
  ...
  return (
    <div className="fixed bottom-4 left-4 right-4 md:left-auto md:right-6 md:max-w-md z-50 animate-in slide-in-from-bottom-4 duration-300">
      <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl shadow-xl p-5">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            <h3 className="font-semibold text-white text-sm mb-1">Nous utilisons des cookies</h3>
          ...
          <button onClick={decline} className="text-white/30 hover:text-white/50 transition-colors shrink-0">
            <X size={16} />
          </button>
        </div>
        <div className="flex gap-2">
          <button
            onClick={decline}
            className="flex-1 py-2 text-xs font-medium text-white/50 border border-white/10 rounded-xl hover:bg-white/5 transition-colors"
          >
            Refuser
          </button>
          <button
